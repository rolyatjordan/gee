...
> Entries below `1.1.0 - March 31, 2022` are inherited from the upstream
> [posh-git](https://github.com/dahlbyk/posh-git) project this module was
> forked from. Function names, module names, and other identifiers have
> changed in `gee` — refer to the current source of truth in [README.md](README.md).

## Unreleased (gee fork)

- Renamed module from `posh-git-redux` to `gee`.
...
- Fixed test-suite leaks: prompt hijack no longer persists after `Invoke-Pester`; `POSHGIT_ENABLE_STRICTMODE` env var no longer leaks; test-created global git aliases now get cleaned up.

---

# posh-git Release History (upstream, pre-fork)
