...
Describe 'ArgumentCompleter registration' {
    It 'Registers git as a native argument completer' {
        # gee uses the modern Register-ArgumentCompleter -Native path on PowerShell 5.x
        # as well as 6+, so the legacy TabExpansion function is not exported.
        # Verify the registered-commands metadata reflects the modern hook.
        $GitTabSettings.RegisteredCommands | Should -Match '\bgit\b'
    }
}
...
Describe 'TabExpansion Tests' {
    ...
    Context 'Fetch/Push/Pull TabExpansion Tests' {
        ...
        BeforeEach {
            # Ensure trunk branch exists
            &$gitbin branch -q trunk 2>$null
            # Ensure an origin remote exists
            &$gitbin remote add origin . 2>$null
            # Ensure origin/trunk exists
            &$gitbin update-ref refs/remotes/origin/trunk $(git rev-parse trunk) 2>$null
            # Ensure origin/HEAD exists
            &$gitbin symbolic-ref refs/remotes/origin/HEAD refs/remotes/origin/trunk 2>$null
        }
        ...
        It 'Tab completes all branches' {
            ...
            $result -contains 'trunk' | Should -Be $true
            $result -contains 'origin/trunk' | Should -Be $true
        ...
        It 'Tab completes matching branches' {
            $result = & $module GitTabExpansionInternal 'git push origin tr'
            $result | Should -BeExactly 'trunk'
        }
        It 'Tab completes matching remote/branches' {
            $result = & $module GitTabExpansionInternal 'git push origin origin/tr'
            $result | Should -BeExactly 'origin/trunk'
        }
        It 'Tab completes matching :branches' {
            $result = & $module GitTabExpansionInternal 'git push origin :tr'
            $result | Should -BeExactly ':trunk'
        ...
        It 'Tab completes matching ref:branches' {
            $result = & $module GitTabExpansionInternal 'git push origin HEAD:tr'
            $result | Should -BeExactly 'HEAD:trunk'
        }
        It 'Tab completes matching +ref:branches' {
            $result = & $module GitTabExpansionInternal 'git push origin +HEAD:tr'
            $result | Should -BeExactly '+HEAD:trunk'
        ...
        It 'Tab completes matching branch with preceding parameters' {
            $result = & $module GitTabExpansionInternal 'git push  --follow-tags  -u   origin tr'
            $result | Should -BeExactly 'trunk'
        }
        It 'Tab completes matching branch with intermixed parameters' {
            $result = & $module GitTabExpansionInternal 'git push -u origin --follow-tags tr'
            $result | Should -BeExactly 'trunk'
            $result = & $module GitTabExpansionInternal 'git push  -u  origin  --follow-tags   tr'
            $result | Should -BeExactly 'trunk'
        }
        It 'Tab completes matching ref:branch with intermixed parameters' {
            $result = & $module GitTabExpansionInternal 'git push -u origin --follow-tags HEAD:tr'
            $result | Should -BeExactly 'HEAD:trunk'
            $result = & $module GitTabExpansionInternal 'git push  -u  origin  --follow-tags   +HEAD:tr'
            $result | Should -BeExactly '+HEAD:trunk'
        }
        It 'Tab completes matching multiple push ref specs with intermixed parameters' {
            $result = & $module GitTabExpansionInternal 'git push -u origin --follow-tags one :two three:four  tr'
            $result | Should -BeExactly 'trunk'
            $result = & $module GitTabExpansionInternal 'git push -u origin --follow-tags one :two three:four  --crazy-param tr'
            $result | Should -BeExactly 'trunk'
            $result = & $module GitTabExpansionInternal 'git push -u origin --follow-tags one :two three:four  HEAD:tr'
            $result | Should -BeExactly 'HEAD:trunk'
            $result = & $module GitTabExpansionInternal 'git push -u origin --follow-tags one :two three:four  --crazy-param HEAD:tr'
            $result | Should -BeExactly 'HEAD:trunk'
        ...
        It 'Tab completes branch names that are symbolic refs' {
            ...
            &$gitbin symbolic-ref refs/heads/$branchName refs/heads/trunk
            try {
    ...
    Context 'Restore Source Branch TabExpansion Tests' {
        ...
        It 'Tab completes source branches -s' {
            $result = & $module GitTabExpansionInternal 'git restore -s tru'
            $result | Should -BeExactly 'trunk'
        }
        It 'Tab completes source branches --source=' {
            $result = & $module GitTabExpansionInternal 'git restore --source=tru'
            $result | Should -BeExactly '--source=trunk'
        }
    }
    ...
    Context 'Git Config Alias TabExpansion Tests' {
        BeforeAll {
            ...
            # Use a List so .Add() mutates the outer collection. Plain += inside a function
            # reassigns a local copy and the tracking list stays empty, leaking aliases.
            $addedAliases = [System.Collections.Generic.List[string]]::new()
        ...
        AfterAll {
            $addedAliases | Where-Object { $_ } | ForEach-Object {
                &$gitbin config --global --unset-all "alias.$_" 2>$null
            }

            # Belt-and-suspenders: sweep any stray 'test-<GUID>' or 'pgtest-<GUID>' aliases
            # left by prior failed runs or the previously-broken tracking logic.
            $guidAliasPattern = '^alias\.(?<name>(pg)?test-[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}) '
            &$gitbin config --global --get-regexp '^alias\.(pg)?test-' 2>$null | ForEach-Object {
                if ($_ -match $guidAliasPattern) {
                    &$gitbin config --global --unset-all "alias.$($Matches['name'])" 2>$null
                }
            }
        ...
        It 'Tab completes when there is one alias of a given name' {
            ...
            $result = & $module GitTabExpansionInternal "git $alias tr"
            $result | Should -BeExactly 'trunk'
        }
        It 'Tab completes when there are multiple aliases of the same name' {
            ...
            $result = & $module GitTabExpansionInternal 'git co tr'
            $result | Should -BeExactly 'trunk'
        }
    }
    ...
    Context 'PowerShell Alias TabExpansion Tests' {
        ...
        It 'Tab completes PowerShell alias specifying git (with no extension)' {
            ...
            $result = & $module GitTabExpansionInternal "g checkout tr"
            $result | Should -BeExactly 'trunk'
        }
        It 'Tab completes PowerShell alias specifying git.exe' {
            ...
            $result = & $module GitTabExpansionInternal "ge checkout tr"
            $result | Should -BeExactly 'trunk'
        }
