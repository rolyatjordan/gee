...
Describe 'Utils Function Tests' {
    Context 'Add-GeeToProfile Tests' {
        BeforeAll {
        ...
        It 'Creates profile file if it does not exist that imports from module path' {
            ...
            Add-GeeToProfile $profilePath $parentDir

            Test-Path -LiteralPath $profilePath | Should -Be $true
            Get-FileEncoding $profilePath | Should -Be $expectedEncoding
            $content = Get-Content $profilePath
            $content.Count | Should -Be 2
            @($content)[1] | Should -BeExactly "Import-Module gee"
        }
        It 'Creates profile file if the profile dir does not exist' {
            ...
            Add-GeeToProfile $childProfilePath

            Test-Path -LiteralPath $childProfilePath | Should -Be $true
            $childProfilePath | Should -FileContentMatch "^Import-Module .*gee"
        }
        It 'Does not modify profile that already refers to gee' {
            $profileContent = @'
Import-Module PSCX
Import-Module gee
'@
            ...
            $output = Add-GeeToProfile $profilePath 3>&1

            $output[1] | Should -Match 'gee appears'
...
'@
            ...
            Add-GeeToProfile $profilePath $moduleBasePath
            ...
            $expectedContent = Convert-NativeLineEnding $profileContent
            $expectedContent += "${newLine}${newLine}Import-Module '$(Join-Path $moduleBasePath gee).psd1'"
            $content -join $newLine | Should -BeExactly $expectedContent
        }
    }

    Context 'Remove-GeeFromProfile Tests' {
        BeforeAll {
            [System.Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseDeclaredVarsMoreThanAssigments', '')]
            $newLine = [System.Environment]::NewLine
        }
...
'@
            ...
            $output = Remove-GeeFromProfile $profilePath 3>&1
    ...
    Context 'Test-GeeImportedInScript Tests' {
        BeforeEach {
            [System.Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseDeclaredVarsMoreThanAssigments', '')]
            $profilePath = [System.IO.Path]::GetTempFileName()
        }
        ...
        It 'Detects Import-Module gee in profile script' {
            $profileContent = "Import-Module gee"
            Set-Content $profilePath -Value $profileContent -Encoding Unicode
            Test-GeeImportedInScript $profilePath | Should -Be $true
        }
        It 'Detects chocolatey installed line in profile script' {
            $profileContent = ". 'C:\tools\poshgit\dahlbyk-gee-18d600a\profile.example.ps1"
            Set-Content $profilePath -Value $profileContent -Encoding Unicode
            Test-GeeImportedInScript $profilePath | Should -Be $true
        }
        It 'Returns false when one-line profile script does not import gee' {
            $profileContent = "# Test"
            Set-Content $profilePath -Value $profileContent -Encoding Unicode
            Test-GeeImportedInScript $profilePath | Should -Be $false
        }
        It 'Returns false when profile script does not import gee' {
    ...
    Context 'Test-InPSModulePath Tests' {
        It 'Returns false for install not under any PSModulePaths' {
            Mock Get-PSModulePath { }
            $path = "C:\Users\Keith\Documents\WindowsPowerShell\Modules\gee\0.7.0\"
            Test-InPSModulePath $path | Should -Be $false
            Should -Invoke Get-PSModulePath -Exactly 1
        }
        It 'Returns true for install under single PSModulePath' {
            Mock Get-PSModulePath {
                return MakeNativePath "$HOME\Documents\WindowsPowerShell\Modules\gee\"
            }
            $path = MakeNativePath "$HOME\Documents\WindowsPowerShell\Modules\gee\0.7.0"
        ...
        It 'Returns true for install under multiple PSModulePaths' {
            Mock Get-PSModulePath {
                return (MakeNativePath "$HOME\Documents\WindowsPowerShell\Modules\gee\"),
                       (MakeNativePath "$HOME\GitHub\dahlbyk\gee\0.6.1.20160330\")
            }
            $path = MakeNativePath "$HOME\Documents\WindowsPowerShell\Modules\gee\0.7.0"
            Test-InPSModulePath $path | Should -Be $true
            Should -Invoke Get-PSModulePath -Exactly 1
        }
        It 'Returns false when current gee module location is not under PSModulePaths' {
            Mock Get-PSModulePath {
                return (MakeNativePath "$HOME\Documents\WindowsPowerShell\Modules\gee\"),
                       (MakeNativePath "$HOME\GitHub\dahlbyk\gee\0.6.1.20160330\")
            }
            $path = MakeNativePath "\tools\gee\dahlbyk-gee-18d600a"
            Test-InPSModulePath $path | Should -Be $false
            Should -Invoke Get-PSModulePath -Exactly 1
        }
        It 'Returns false when current gee module location is under PSModulePath, but in a src directory' {
            Mock Get-PSModulePath {
                return MakeNativePath '\GitHub'
            }
            $path = MakeNativePath "\GitHub\gee\src"
            Test-InPSModulePath $path | Should -Be $false
            Should -Invoke Get-PSModulePath -Exactly 1
        }
    }
