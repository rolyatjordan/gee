...
Describe 'Get-GitDiretory Tests' {
    ...
    Context 'Test worktree' {
        BeforeEach {
            ...
            # Current Git versions require the worktree path not to exist; Git
            # creates it as part of `worktree add`.
            $sourceBranch = &$gitbin branch --show-current
            &$gitbin worktree add -b test-worktree $worktreePath $sourceBranch 2>$null
            if ($LASTEXITCODE -ne 0) {
                throw "Failed to create test worktree at $worktreePath."
            }
        }
        AfterEach {
            Set-Location $origPath
    ...
    Context "Test GIT_DIR environment variable" {
        ...
        It 'Returns the value in GIT_DIR env var' {
            $env:GIT_DIR = MakeNativePath '/xyzzy/gee/.git'
