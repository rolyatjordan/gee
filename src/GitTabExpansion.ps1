...
$Global:GitTabSettings = New-Object PSObject -Property @{
    ...
    KnownAliases = @{}
    EnableLogging = $false
    LogPath = Join-Path ([System.IO.Path]::GetTempPath()) gee_tabexp.log
    RegisteredCommands = ""
    # Regex matched against alias names in gitAliases(). Matches are dropped from tab
    # completion. Default targets 'test-<GUID>' probe-and-forget aliases some tools leave
    # in ~/.gitconfig. Set to '' to disable filtering.
    ExcludeAliasPattern = '^test-[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$'
}
...
$script:someCommands = @('add','branch','checkout','cherry-pick','clone','commit','diff','fetch','log','merge',
                         'pull','push','rebase','reset','restore','revert','stash','status','switch','tag','worktree')
...
function script:gitAliases($filter) {
    $excludePattern = $global:GitTabSettings.ExcludeAliasPattern
    git config --get-regexp ^alias\. | ForEach-Object{
        if ($_ -match "^alias\.(?<alias>\S+) .*") {
            $alias = $Matches['alias']
            if ($alias -like "$filter*" -and (-not $excludePattern -or $alias -notmatch $excludePattern)) {
                $alias
            }
        }
    } | Sort-Object -Unique
...
if (!$UseLegacyTabExpansion -and ($PSVersionTable.PSVersion.Major -ge 5)) {
    ...
    # Create regex pattern from $cmdNames: ^(git|git\.exe|gitk|gitk\.exe)$
