...
function Invoke-Utf8ConsoleCommand([ScriptBlock]$cmd) {
    ...
    try {
        ...
        if ($currentEncoding.EncodingName -ne [Text.Encoding]::UTF8.EncodingName) {
...
<#
.SYNOPSIS
    Configures your PowerShell profile (startup) script to import the gee
    module when PowerShell starts.
.DESCRIPTION
    Checks if your PowerShell profile script is not already importing gee
    and if not, adds a command to import the gee module. This will cause
    PowerShell to load gee whenever PowerShell starts.
.PARAMETER AllHosts
...
.PARAMETER Force
    ...
    gee. Just add Import-Module gee command.
.EXAMPLE
    PS C:\> Add-GeeToProfile
    Updates your profile script for the current PowerShell host to import the
    gee module when the current PowerShell host starts.
.EXAMPLE
    PS C:\> Add-GeeToProfile -AllHosts
    Updates your profile script for all PowerShell hosts to import the gee
...
function Add-GeeToProfile {
    ...
    if ($AllUsers -and !(Test-Administrator)) {
        throw 'Adding gee to an AllUsers profile requires an elevated host.'
    }
    ...
    if ($TestParams -and ($TestParams.Count -gt 0) -and ($TestParams[0] -is [string])) {
    ...
    if (!$Force) {
        # Search the user's profiles to see if any are using gee already, there is an extra search
        # ($profilePath) taking place to accommodate the Pester tests.
        $importedInProfile = Test-GeeImportedInScript $profilePath
        if (!$importedInProfile -and !$underTest) {
            $importedInProfile = Test-GeeImportedInScript $PROFILE
        }
        if (!$importedInProfile -and !$underTest) {
            $importedInProfile = Test-GeeImportedInScript $PROFILE.CurrentUserCurrentHost
        }
        if (!$importedInProfile -and !$underTest) {
            $importedInProfile = Test-GeeImportedInScript $PROFILE.CurrentUserAllHosts
        }
        if (!$importedInProfile -and !$underTest) {
            $importedInProfile = Test-GeeImportedInScript $PROFILE.AllUsersCurrentHost
        }
        if (!$importedInProfile -and !$underTest) {
            $importedInProfile = Test-GeeImportedInScript $PROFILE.AllUsersAllHosts
        }

        if ($importedInProfile) {
            Write-Warning "Skipping add of gee import to file '$profilePath'."
            Write-Warning "gee appears to already be imported in one of your profile scripts."
    ...
    if (!$profilePath) {
        Write-Warning "Skipping add of gee import to profile; no profile found."
    ...
    if (Test-Path -LiteralPath $profilePath) {
        if (!(Get-Command Get-AuthenticodeSignature -ErrorAction SilentlyContinue))
        {
            ...
            if ($null -ne $sig.SignerCertificate) {
                Write-Warning "Skipping add of gee import to profile; '$profilePath' appears to be signed."
                Write-Warning "Add the command 'Import-Module gee' to your profile and resign it."
                return
            }
        }
    }
    ...
    if (Test-InPSModulePath $ModuleBasePath) {
        $profileContent = "`nImport-Module gee"
    }
    else {
        $modulePath = Join-Path $ModuleBasePath gee.psd1
    ...
    if ($PSCmdlet.ShouldProcess($profilePath, "Add 'Import-Module gee' to profile")) {
...
.SYNOPSIS
    ...
    the gee module when PowerShell starts.
.DESCRIPTION
    Checks if your PowerShell profile script is importing gee and if it does,
    removes the command to import the gee module. This will cause PowerShell
    to no longer load gee whenever PowerShell starts.
.PARAMETER AllHosts
    By default, this command modifies the CurrentUserCurrentHost profile
    script.  By specifying the AllHosts switch, the command updates the
    CurrentUserAllHosts profile (or AllUsersAllHosts, given -AllUsers).
.PARAMETER AllUsers
    ...
    Requires elevated permissions.
.EXAMPLE
    PS C:\> Remove-GeeFromProfile
    Updates your profile script for the current PowerShell host to stop importing
    the gee module when the current PowerShell host starts.
.EXAMPLE
    PS C:\> Remove-GeeFromProfile -AllHosts
    Updates your profile script for all PowerShell hosts to no longer import the
    gee module whenever any PowerShell host starts.
.INPUTS
    None.
.OUTPUTS
    None.
#>
function Remove-GeeFromProfile {
    [CmdletBinding(SupportsShouldProcess)]
    param(
        [Parameter()]
        [switch]
    ...
    if ($AllUsers -and !(Test-Administrator)) {
        throw 'Removing gee from an AllUsers profile requires an elevated host.'
    }
    ...
    if (!$profilePath) {
        Write-Warning "Skipping removal of gee import from profile; no profile found."
        Write-Verbose "`$PROFILE              = '$PROFILE'"
        Write-Verbose "CurrentUserCurrentHost = '$($PROFILE.CurrentUserCurrentHost)'"
        Write-Verbose "CurrentUserAllHosts    = '$($PROFILE.CurrentUserAllHosts)'"
        Write-Verbose "AllUsersCurrentHost    = '$($PROFILE.AllUsersCurrentHost)'"
    ...
    if (Test-Path -LiteralPath $profilePath) {
        ...
        if (!(Get-Command Get-AuthenticodeSignature -ErrorAction SilentlyContinue))
        {
            ...
            $sig = Get-AuthenticodeSignature $profilePath
            if ($null -ne $sig.SignerCertificate) {
                Write-Warning "Skipping removal of gee import from profile; '$profilePath' appears to be signed."
                Write-Warning "Remove the command 'Import-Module gee' from your profile and resign it."
                return
            }
        }
        ...
        foreach($line in $oldProfile) {
            if ($line -like '*Load gee example profile*') { continue; }

            if($line -like '. *gee*profile.example.ps1*') {
                continue;
            }
            if($line -like 'Import-Module *\gee.psd1*') {
                continue;
            }
            $newProfile += $line
        }
...
function Test-InPSModulePath {
    ...
    if ($inModulePath -and ('src' -eq (Split-Path $Path -Leaf))) {
        Write-Warning 'gee repository structure is incompatible with %PSModulePath%.'
...
function Test-GeeImportedInScript {
    param (
    ...
    $match = (@(Get-Content $Path -ErrorAction SilentlyContinue) -match 'gee').Count -gt 0
    if ($match) { Write-Verbose "gee found in '$Path'" }
    $match
}
