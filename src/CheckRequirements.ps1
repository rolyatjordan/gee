...
function Test-GitVersion ($version = $([string](git --version 2> $null))) {
    if ($version -notmatch '(?<ver>\d+(?:\.\d+)+)(?<g4w>(?<rc>[-.]rc\d+)?\.windows|\.vfs)?') {
        Write-Warning "gee could not parse Git version ($version)"
    ...
    if ((($PSVersionTable.PSVersion.Major -le 5) -or $IsWindows) -and !$Matches['g4w']) {
        ...
        if (!$Env:GEE_CYGWIN_WARNING) {
            Write-Warning 'You appear to have an unsupported Git distribution. gee recommends Git for Windows.'
        }
    }
...
if (!(Test-GitVersion)) {
    Write-Warning "gee requires Git $requiredVersion or better. You have $GitVersion."
}
