...
.EXAMPLE
    PS C:\GitHub\gee\tests> Get-GitDirectory
    Returns C:\GitHub\gee\.git
.INPUTS
    None.
.OUTPUTS
    System.String
                ...
                { '({0})' -f (Invoke-NullCoalescing `
                    {
                        ...
                        switch ($Global:GeeStatusSettings.DescribeStyle) {
...
<#
.SYNOPSIS
    Returns a structured object describing the current Git repository.
.DESCRIPTION
    Get-GitStatus returns a PSObject with the current branch, ahead/behind
    counts, staged/unstaged file counts, stash count, and related state.
    External prompt engines (e.g. Oh My Posh) or custom `prompt` functions
    can consume it directly.

    Properties on $GeeStatusSettings tune what data is gathered:

    EnableFileStatus          = $true  # Set $false to skip file counts
    ...
    The -Force parameter overrides EnableFileStatus and EnablePromptStatus
    when they are $false so all status information is returned regardless.
.EXAMPLE
    PS C:\> $s = Get-GitStatus
    Returns the status object for the repository at the current location.
.INPUTS
    None
.OUTPUTS
...
function Get-GitStatus {
    param(
        ...
        # If specified, overrides $GeeStatusSettings.EnableFileStatus and
        # $GeeStatusSettings.EnablePromptStatus when they are set to $false.
        [Parameter()]
        [switch]
        $Force
    )

    $settings = if ($global:GeeStatusSettings) { $global:GeeStatusSettings } else { [GeeStatusSettings]::new() }
...
function InDisabledRepository {
    ...
    foreach ($repo in $Global:GeeStatusSettings.RepositoriesInWhichToDisableFileStatus) {
...
.DESCRIPTION
    ...
    * main
    ...
    IMPORTANT: Be careful using this command. Even though by default this command deletes only merged branches, most, if not all, of your historical branches have been merged. But that doesn't mean you want to delete them. That is why this command excludes `develop`, `trunk`, and `main` by default. But you may use different names e.g. `development` or have other historical branches you don't want to delete. In these cases, you can either:

    * Specify a narrower branch name wildcard such as "user/$env:USERNAME/*".
    * Specify an updated ExcludeParameter e.g. '(^\*)|(^. (develop|trunk|main|v\d+)$)' which adds any branch matching the pattern 'v\d+' to the exclusion list.
...
.EXAMPLE
    PS> Remove-GitBranch -Name * -ExcludePattern '(^\*)|(^. (develop|trunk|main|v\d+)$)'
    Deletes merged branches except the current branch, develop, trunk, main, and branches that also match the pattern 'v\d+' e.g. v1, v1.0, v1.x. BE VERY CAREFUL SPECIYING SUCH A BROAD BRANCH NAME WILDCARD!
.EXAMPLE
...
function Remove-GitBranch {
    ...
    param(
        # Specifies a regular expression pattern for the branches that will be deleted. Certain branches are always excluded from deletion e.g. the current branch as well as the develop, trunk, and main branches. See the ExcludePattern parameter to modify that pattern.
        [Parameter(Position = 0, Mandatory, ParameterSetName = "Wildcard")]
        [ValidateNotNullOrEmpty()]
        [string]
        $Name,

        # Specifies a regular expression for the branches that will be deleted. Certain branches are always excluded from deletion e.g. the current branch as well as the develop, trunk, and main branches. See the ExcludePattern parameter to modify that pattern.
        [Parameter(Position = 0, Mandatory, ParameterSetName = "Pattern")]
        [ValidateNotNull()]
        [string]
        $Pattern,

        # Specifies a regular expression used to exclude merged branches from being deleted. The default pattern excludes the current branch, develop, trunk, and main branches.
        [Parameter()]
        [ValidateNotNull()]
        [string]
        $ExcludePattern = '(^\*)|(^. (develop|trunk|main)$)',
...
function Update-AllBranches($Upstream = 'trunk', [switch]$Quiet) {
